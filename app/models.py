...
class UsageHelp(db.Model):
  ...
  def __repr__(self):
    return f"<Usage Help: {self.title}>"
...
class SubCriteria(db.Model):
  ...
  def __repr__(self):
    return f"<Sub Criteria: {self.name}>"
...
class TourDistance(db.Model):
  ...
  def __repr__(self):
    return f"<Tour Distance: {self.distance} km to {self.location_point.name}>"